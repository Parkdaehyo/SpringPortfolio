...
public class PagingAlgorithmTest {
	 ...
	 *** 페이징 알고리즘 만들기
	 ...
	 ex) 끝페이지가 40개여서 *10 한 수= 400 >= 총 개시물 수가 324개이라면, 비활성
	...
	public void pagingAlgorithmTest() {
		...
		boolean isPrev = (beginPage == 1) ? false : true; //1이면 비활성하겠다.
	...
	public void searchTest() {
		...
		search.setPage(3); //PageVO를 상속받았기 때문에 부모의 멤버변수를 쓸 수 있음.
		search.setKeyword("포트폴리오"); //SQL문의 keyword 부분.
		
		System.out.println("-----------------------");
		//mapper.getArticleListByTitle(search) //mapper.xml과 연결
			  //.forEach(vo -> System.out.println(vo)); //연결된 구문과 반복 작업.
		System.out.println("-------------------------"); //1페이지에 존재하는 "포트폴리오"가 들어간 모든 글 출력.
...
/*
 * 논리반전 !(Not)연산자 현재 값이 true이면, 결과값은 false이고, 현재값이 false이면, 결과값이 true아.
 * 
 */
